...
class Squirrel_REST_Posts_Controller  extends WP_REST_Controller{
    ...
    public function register_routes() {
        register_rest_route( $this->namespace, '/' . $this->resource_name.'/banners', array(
            // Here we register the readable endpoint for collections.
            array(
                'methods'   => 'GET',
                'callback'  => array( $this, 'getPostBanners' ),
    ...
    function getTopPraisePostsThisYear($request)
    {
       ...
       // $fristday = date( "Y-m-d H:i:s",  strtotime(date("Y",time())."-1"."-1"));  //本年第一天;
        ...
        $sql="SELECT  ".$wpdb->posts.".ID as ID, post_title, post_name,post_content,post_author,post_date, count(".$wpdb->postmeta.".post_id) AS 'praise_total' FROM ".$wpdb->posts." LEFT JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id WHERE ".$wpdb->postmeta.".meta_value like '%praise' AND post_date BETWEEN '".$fristday."' AND '".$today."' AND post_status = 'publish' and  post_type='post' AND post_password = '' GROUP BY ".$wpdb->postmeta.".post_id ORDER  BY praise_total DESC LIMIT ". $limit;
        ...
        global $wpdb, $post, $tableposts, $tablecomments, $time_difference, $post;
           ...
           // $fristday = date( "Y-m-d H:i:s",  strtotime(date("Y",time())."-1"."-1"));  //本年第一天;
            ...
            foreach ($mostlikes as $post) {
                    ...
                    $_data["post_permalink"] =$post_permalink;                    
                    //添加头像
                    $_data["post_author"] =$post->post_author;
                    $u = get_user_by('ID',$post->post_author);

                    $_data['avatar'] = 'https://www.squirrelzoo.com/wp-content/uploads/2019/06/2019061105290828-150x150.png';
                    $xxx  = get_user_meta($post->post_author,'squirrelzoo_avatar',true);
                    if (!empty($xxx)){
                        $_data['avatar'] = $xxx;
                    }
                    $_data['nickname'] = $u->display_name;
        ...
        foreach ($mostcommenteds as $post) {
                ...
                $_data["post_permalink"] =$post_permalink;
                 //添加头像
                  $_data["post_author"] =$post->post_author;
                $u = get_user_by('ID',$post->post_author);
                $_data['avatar'] = get_user_meta($post->post_author,'squirrelzoo_avatar',true);
                $_data['nickname'] = $u->display_name;
                $category =get_the_category($post_id);
                if(!empty($category))
                {
    ...
    function getPostBanners($request) {    
    
        global $wpdb;
        $postSwipeIDs = get_option('squirrelzoo_banners');
        $posts =array();                  
        if(!empty($postSwipeIDs))
        {
            $sql="SELECT *  from ".$wpdb->posts." where id in(".$postSwipeIDs.") order by field(id,".$postSwipeIDs.");";
            $_posts = $wpdb->get_results($sql);
...
(SELECT post_id from ".$wpdb->postmeta." where meta_value='like' and meta_key='_".$openid."') ORDER BY post_date desc LIMIT 20";        
    ...
    public function hasUser($openid){
        global $wpdb;
        $sqlx = "select * from ".$wpdb->usermeta." where meta_key='squirrelzoo_openid' and meta_value='".$openid."'";
        $c = count($wpdb->get_results($sqlx));
        return $c > 0;
    }
